...
export function TeamBayern({ teamName, location, logo, socialLinks, players }: TeamBayernProps) {
  return (
    <div className="rounded-md bg-[#1a1a1a] h-full">
      <div className="p-4 flex items-center">
        <div className="mr-4">
          <img src={logo || "/bayern-logo.svg"} alt={teamName} className="h-12 w-12" />
        </div>
        <div>
        ...
        {players.map((player, index) => (
          <div key={index} className="flex items-center px-4 py-2 border-b border-[#333]">
            <div className="h-8 w-8 rounded-full bg-[#222] flex items-center justify-center mr-3">
              <img src={`/player-${index + 1}.jpg`} alt={player.name} className="h-8 w-8 rounded-full" />
            </div>
